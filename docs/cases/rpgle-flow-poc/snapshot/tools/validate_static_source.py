...
# Imported evidence is pinned independently of the live skill library.
# The historical skill manifest is retained as provenance, not a local install.
manifest=json.loads((ROOT.parent/'provenance.json').read_text())
for item in manifest['files']:
 p=(ROOT.parent/item['path']).resolve()
 if not p.is_relative_to(ROOT.parent.resolve()):
  check(False,'invalid imported path '+item['path']);continue
 check(p.is_file(),'missing imported file '+item['path'])
 if p.is_file():
  check(hashlib.sha256(p.read_bytes()).hexdigest()==item['imported_sha256'],'imported hash '+item['path'])
