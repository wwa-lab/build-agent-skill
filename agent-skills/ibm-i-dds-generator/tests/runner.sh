...
#
# Uses `agent -p` to invoke the ibm-i-dds-generator skill for each test case,
...
#
# Output:
#   results/tc-XX-actual.txt     — Raw agent output per test
...
if [[ -z "$AGENT_BIN" && "$LIST_ONLY" == "false" && "$DRY_RUN" == "false" ]]; then
...
while [[ $# -gt 0 ]]; do
  case "$1" in
    ...
    --help|-h)
      ...
      echo "  --model NAME  Agent model to use (default: sonnet)"
...
# --- Check agent is available for execution ---
if [[ -z "$AGENT_BIN" ]]; then
  echo "ERROR: agent CLI not found. Set AGENT_BIN=/path/to/agent or add agent to PATH."
  exit 1
fi
...
for tc_id in "${CASES[@]}"; do
  ...
  set +e
  actual_output=$(echo "$prompt" | "$AGENT_BIN" -p \
    --model "$MODEL" \
    2>/dev/null)
  exit_code=$?
  set -e
  ...
  if [[ $exit_code -ne 0 ]]; then
    echo -e "${RED}ERROR${NC} (agent exit code: ${exit_code})"
